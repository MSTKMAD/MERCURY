...
const int TRAMOS = 10;
const int curva_0[20] = {20, 3, 20, 6, 20, 7, 20, 8, 20, 11, 20, 11, 20, 8, 20, 7, 20, 6, 20, 3};

const int speed_9volt = 110;
const int tolerancia_pid = 3;
int32_t current_target = 0;
const int r_motor = 16;
bool active_pid = false;
bool one_time = false;
    ...
    if (timer_sense.poll(100) != C_TIMER_NOT_EXPIRED)
    {
        ...
        */
        ...
        if ((index_curva == 0) && (resistance == 0) && (one_time == false))
        {
            ...
            current_target = 56;
            Vreal_target = 40 / 10 - r_motor * 46 / 1000;
            active_pid = true;
        }
...
void PIDControl()
{
    static float u_control = 90;
    float Kp = 1.5;  // 1
    float Ki = 0.03; // 0.03
    float Kd = 0.02; // 0.02
    int delta = 30;  // ms
    static int prev_increment = 0;
    float I, P, D = 0;
    float increment = 0;
    float error = 0;
    ...
    if (timer_delta_pid.poll(delta) != C_TIMER_NOT_EXPIRED)
    {
        if ((volts == 40) && (active_pid == true))
        {
            SenseCurrent();
            ...
            // Serial.printf("Vt:%0.1f Vm:%0.1f U:%0.2f Err:%d P:%0.2f I:%0.2f D:%0.2f\n",
            //  Vreal_target, Vreal_motor, u_control * 10,
            //(int)error, P, I, D);
            u_control = u_control * 10;
            SetVoltage((int)u_control);
