...
const uint16_t C_PIN_SS = 7;       // SS Display Dummy value;
...
// Control de Salida
const int C_PIN_PALANCA_OUTPUT = 11;
const int C_OUTPUT_1 = 0x01;
const int C_OUTPUT_2 = 0x02;
const int C_OUTPUT_NONE = 0x03;
int palanca_output = C_OUTPUT_NONE;

// Tracking
const int PALANCA_TRACK = 10;

bool prev_palanca_track = false;
bool actual_palanca_track = false;

int TRACK_MODE_PIN = 15;

bool prev_track_mode_pin = false;
bool actual_track_mode_pin = false;

const int TRACK_LED = 5;
...
int SAVE_MODE_PIN = 14;

bool prev_save_mode_pin = false;
bool actual_save_mode_pin = false;

int read_freq_card = 0;
int read_duty_card = 0;
...
int t0;
int dma_channel;
bool display_state_freq = false;
bool display_state_duty = false;
void setup()
{
    Wire.begin();
    pinMode(17, OUTPUT);
    ...
    mcp.portMode(MCP23017Port::B, 0); // Port A as output

    delay(1000);
    mcp.writeRegister(MCP23017Register::GPIO_A, 0xFF); // Reset port A
    mcp.writeRegister(MCP23017Register::GPIO_B, 0xFF); // Reset port B
    ...
    adc_init();
    adc_gpio_init(28);   // Configurar GPIO 26 (ADC0) como entrada
    adc_select_input(2); // Seleccionar canal ADC0
    adc_set_clkdiv(4800);
...
void core1_entry()
{
    while (true)
    {
        ...
        if ((actual_track_mode_pin == false) && (prev_track_mode_pin == true))
        {
            ...
            mcp.digitalWrite(TRACK_LED, !track_mode);
        }

        if (track_mode == true)
        {
            if (digitalRead(PALANCA_TRACK))
            {
                is_connected = ee.begin();
                Serial.println(is_connected);

                read_freq_card = ee.readByte(0);
                read_duty_card = ee.readByte(20);
                Serial.println(read_freq_card);
                Serial.println(read_duty_card);

                frecuencia_min = constrain(read_freq_card - 20, 0, 250);
                frecuencia_max = constrain(read_freq_card + 20, 0, 250);
                duty_min = constrain(read_duty_card - 20, 0, 100);
                duty_max = constrain(read_freq_card + 20, 0, 100);
            }
            else
            {
        ...
        prev_save_mode_pin = actual_save_mode_pin;
        actual_save_mode_pin = digitalRead(SAVE_MODE_PIN);

        if ((actual_save_mode_pin == false) && (prev_save_mode_pin == true))
        {
            is_connected = ee.begin();
            Serial.println(is_connected);
            Serial.println(ee.writeByte(0, frecuencia));
            Serial.println(ee.writeByte(20, dutyCyclePromedio));
        }

        /*
         if (digitalRead(palanca_machine))
         {
             // DK MODE
        ...
        */
        if (update_display == true)
        {
            ...
            display.setTextColor(WHITE);
            display.setCursor(0, 16);
            display.print((int)dutyCyclePromedio);
            display.setTextSize(3);
            display.setCursor(80, 23);
            display.print("%");
            display.display();
